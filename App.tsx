...
function App() {
  ...
  // --- ESCUCHA DE SUPABASE ---
  const { syncSale, syncSettings, syncClosure, refreshData } = useSupabaseSync(
    settings,
    setSettings,
    reports,
    setReports,
        ...
        <div className="h-full w-full bg-white rounded-[38px] flex flex-col relative overflow-hidden" style={{ backgroundColor: 'var(--page-bg-color)' }}>
          {currentView !== 'kitchen' && (
            <div className="bg-white border-b px-4 py-3 flex justify-around items-center shrink-0">
              ...
              <button onClick={handleManualSync} className={`flex flex-col items-center gap-1 ${isSyncing ? 'text-brand' : 'text-gray-400'}`}>
